...
async function generateWithOpenAI(questionType, studentId, avoidTopics, attempt = 1) {
  ...
  // Consume this seed for subsequent retries in the same request.
  avoidTopics.push(assigned.topic, assigned.domain);

  let userPrompt = buildGenerationPrompt(questionType, avoidTopics, assigned);
  if (attempt > 1) {
    userPrompt += `
