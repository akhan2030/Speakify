...
import {
  RECENT_TOPIC_WINDOW,
  assertFollowsAssignedTopic,
  assertTopicIsDiverse,
  isNearDuplicateTopic,
  pickAssignedTopic,
} from "./readingTopicDiversity.js";

const MODEL = "gpt-4o-mini";
/** Creative enough for unique topics; factual accuracy is enforced by validation, not temperature. */
const TEMPERATURE = 0.85;
/** Server-side retries so invalid / non-diverse drafts never reach the student. */
 ...
 * @param {string[]} avoidTopics
 * @param {{ domain: string, topic: string }} assigned
 */
function buildGenerationPrompt(questionType, avoidTopics, assigned) {
  const type = normalizeQuestionType(questionType);
  const avoidList =
    avoidTopics.length > 0
      ? avoidTopics.slice(0, RECENT_TOPIC_WINDOW).join("; ")
      : "(none)";
...
ASSIGNED TOPIC (mandatory — do not substitute a different subject):
- Domain: ${assigned.domain}
- Topic: ${assigned.topic}
Write the entire passage about this assigned topic only. Invent a fresh academic title that reflects THIS topic.
Do NOT default to urbanization, biodiversity, renewable energy overview, or the water cycle unless the assigned topic explicitly is that subject.

AVOID — do not reuse or thinly rewrite any of these recent titles/topics:
${avoidList}

Return ONLY valid JSON (no markdown) in this exact shape:
{
  "title": "string (unique; must reflect the assigned topic)",
  "topic": "${assigned.topic}",
...
- Passage describing a clear process, cycle, or labeled structure (5–7 steps/parts) tied to the ASSIGNED topic (not a generic water-cycle default)
- "diagram": {
    "title": "string",
...
async function getPreviousTopics(supabase, studentId) {
  ...
  for (const row of passages ?? []) {
    ...
    if (row.title) topics.add(String(row.title));
  }
  return [...topics];
}

/**
 * Global recent titles/topics across all students (last N bank rows).
 * @param {import("@supabase/supabase-js").SupabaseClient} supabase
 */
async function getRecentGlobalTopics(supabase) {
  const { data, error } = await supabase
    .from("passage_bank")
    .select("topic, title")
  ...
  if (error) {
    console.warn("[passageGenerator] global topics read:", error.message);
    return [];
  }
...
/**
 * Merge student + global avoid lists (deduped, capped).
 * @param {string[]} studentTopics
 * @param {string[]} globalTopics
 * @param {string[]} [sessionTopics] — topics already generated in this request chain
 */
function mergeAvoidTopics(studentTopics, globalTopics, sessionTopics = []) {
  const out = [];
  const seen = new Set();
  for (const raw of [...sessionTopics, ...studentTopics, ...globalTopics]) {
    ...
    seen.add(norm);
    out.push(key);
    if (out.length >= RECENT_TOPIC_WINDOW * 2) break;
  }
  return out;
}
 ...
 * @param {string[]} [avoidTopics]
 */
async function findUnusedPassage(supabase, questionType, seenIds, avoidTopics = []) {
  const { data, error } = await supabase
    .from("passage_bank")
    .select("*")
  ...
  for (const row of data ?? []) {
    ...
    if (
      isNearDuplicateTopic(String(row.topic ?? ""), avoidTopics) ||
      isNearDuplicateTopic(String(row.title ?? ""), avoidTopics)
    ) {
      continue;
    }
 ...
 */
async function generateWithOpenAI(questionType, studentId, avoidTopics, attempt = 1) {
  const openai = getOpenAI();
  if (!openai) {
    throw new Error("OpenAI is not configured");
  }

  const assigned = pickAssignedTopic({ avoid: avoidTopics });
  // Prompt avoid-list includes prior topics only — do NOT add `assigned` until after success,
  // or assertTopicIsDiverse will reject the passage for matching its own assigned seed.
  const promptAvoid = [...avoidTopics];

  let userPrompt = buildGenerationPrompt(questionType, promptAvoid, assigned);
  if (attempt > 1) {
    userPrompt += `

RETRY ${attempt}/${MAX_GENERATION_ATTEMPTS}: Previous draft failed validation OR topic diversity checks.
Stay on the ASSIGNED topic above. Do not fall back to urbanization/biodiversity/water cycle/renewable overview defaults.
...
Example answer pattern that PASSES: A, C, A, B, C, F, C (A and C reused; D/E/G unused).`;
  ...
  const completion = await openai.chat.completions.create({
    model: MODEL,
    temperature: Math.min(1, TEMPERATURE + (attempt - 1) * 0.05),
    messages: [
      {
        role: "system",
        content:
          "You are an expert IELTS Academic Reading test writer. Output only valid JSON. Always follow the assigned topic exactly.",
      },
      {
        role: "user",
        content: userPrompt,
  ...
  const generated = {
    ...
    topic: String(parsed.topic ?? assigned.topic),
  ...
  // Prefer assigned topic key for bank tracking even if the model paraphrases slightly.
  if (!generated.topic || generated.topic === "General Academic") {
    generated.topic = assigned.topic;
  }

  assertTopicIsDiverse(generated, avoidTopics);
  assertFollowsAssignedTopic(generated, assigned);
  // Keep bank/history keyed to the assigned seed for future avoid lists.
  generated.topic = assigned.topic;
  // Reserve this seed for later attempts / later requests in the same chain.
  avoidTopics.push(assigned.topic, assigned.domain, generated.title);
...
async function generateWithOpenAIRetry(questionType, studentId, avoidTopics) {
  let lastError = null;
  /** @type {string[]} */
  const sessionAvoid = [...avoidTopics];
  for (let attempt = 1; attempt <= MAX_GENERATION_ATTEMPTS; attempt += 1) {
    try {
      ...
      // Prevent same-request retries from re-picking a near-identical topic.
      if (generated.topic) sessionAvoid.push(String(generated.topic));
      if (generated.title) sessionAvoid.push(String(generated.title));
      return generated;
    } catch (err) {
      lastError = err;
      console.warn(
...
export async function generatePassage(
  ...
  const studentTopics = await getPreviousTopics(supabase, studentId);
  const globalTopics = await getRecentGlobalTopics(supabase);
  const avoidTopics = mergeAvoidTopics(studentTopics, globalTopics);

  let bankRow = await findUnusedPassage(supabase, slug, seenIds, avoidTopics);
