...
function extractScoresFromText(text, taskType = "task2") {
  const firstLabel = taskType === "task1" ? "TA" : "TR";
  const firstRe = new RegExp(
    `${firstLabel}\\s*:\\s*([0-9]+(?:\\.[05])?)\\s*\\/\\s*9`,
    "i"
  );
  const ta =
    text.match(firstRe)?.[1] ??
    text.match(/TA\s*:\s*([0-9]+(?:\.[05])?)\s*\/\s*9/i)?.[1];
...
Use IELTS General Training Writing Task 2 criteria:
...
export async function evaluateGeneralWriting({
  ...
  const bands = extractScoresFromText(evaluation, taskType);
