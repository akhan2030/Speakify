...
/** @deprecated Prefer per-block size from getSectionQuestionBlocks — plan is no longer always 5+5. */
...
export function validateSectionQuestions(questions, sectionNumber) {
  ...
  for (const block of blocks) {
    const expectedCount = block.end - block.start + 1;
    const inBlock = questions.filter(
      (q) => q.questionNumber >= block.start && q.questionNumber <= block.end
    );
    if (inBlock.length !== expectedCount) {
      errors.push(
        `Block ${block.start}–${block.end}: expected ${expectedCount}, got ${inBlock.length}`
      );
    }
  }
