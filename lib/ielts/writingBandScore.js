...
function extractScoreHeaderBlock(text) {
  const lines = String(text ?? "").split("\n");
  const headerLines = [];

  for (const line of lines) {
    const trimmed = line.trim();
    if (/^(TA|TR|CC|LR|GRA)\s*:/i.test(trimmed) || /^Overall Band\s*:/i.test(trimmed)) {
...
function parseCriterionFromText(text, label) {
  ...
  const header = extractScoreHeaderBlock(text) || text.slice(0, 600);
...
export function syncWritingEvaluationScores(evaluationText, taskType = "task2") {
...
export function resolveWritingOverallBand(
  bands,
  evaluationText = "",
  taskType = "task2"
) {
  if (evaluationText) {
    const parsed = finalizeWritingBands(evaluationText, taskType);
    if (parsed.overall != null) return parsed.overall;
  }

  const fromCriteria = calculateWritingOverallBand(
    bands?.ta ?? null,
    bands?.cc ?? null,
    bands?.lr ?? null,
    bands?.gra ?? null
  );
  if (fromCriteria != null) return fromCriteria;

  if (bands?.overall != null && Number.isFinite(Number(bands.overall))) {
    return Number(bands.overall);
  }
