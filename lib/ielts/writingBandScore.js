/**
 * IELTS Academic Writing band parsing and overall score calculation (TA, CC, LR, GRA).
 */

const CRITERION_KEYS = ["ta", "cc", "lr", "gra"];
const CRITERION_LABELS = { ta: "TA", cc: "CC", lr: "LR", gra: "GRA" };
...
export function parseWritingCriterionScores(text) {
  return {
    ta: parseCriterionFromText(text, "TA"),
...
export function finalizeWritingBands(evaluationText) {
  const parsed = parseWritingCriterionScores(evaluationText);
...
export function rewriteEvaluationScores(evaluation, bands) {
  ...
  const body = evaluation
    .split("\n")
    .filter((line) => !/^(TA|CC|LR|GRA)\s*:/i.test(line.trim()))
...
export { CRITERION_KEYS, CRITERION_LABELS };
