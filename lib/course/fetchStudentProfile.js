...
export async function fetchStudentProfile(studentId) {
  ...
  const [
    ...
    supabase.from("users").select("target_band, cefr_level, accelerator_track, checkout_track").eq("id", studentId).maybeSingle(),
    supabase
      .from("writing_attempts")
      .select("band_overall, created_at")
  ...
  const writing = averageBand((writingRes.data ?? []).map((r) => Number(r.band_overall)));
  const speaking = averageBand((speakingRes.data ?? []).map((r) => Number(r.band_overall)));
  const speakingFromProgress = roundBand(speakingProgressRes.data?.current_band);
  const speakingFromSessions = averageBand(
    (speakingSessionsRes.data ?? []).map((r) => Number(r.overall_band))
  );
  const speakingBand =
    speaking ?? speakingFromProgress ?? speakingFromSessions;
  ...
  const checkoutTrack = userRes.data?.checkout_track ?? null;
  ...
  const activityTimestamps = [
    ...
    ...(speakingSessionsRes.data ?? []).map((r) => r.completed_at),
  ...
  return buildStudentProfile({
    studentId,
    skillBands: { writing, speaking: speakingBand, reading, listening, vocabulary, grammar },
    targetBand,
    placementBand,
    acceleratorTrack,
