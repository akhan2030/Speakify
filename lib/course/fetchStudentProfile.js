...
export async function fetchStudentProfile(studentId) {
  ...
  const [
    ...
    supabase.from("users").select("target_band, cefr_level").eq("id", studentId).maybeSingle(),
    supabase
    ...
    supabase
      ...
      .select("level_id, target_band, levels(name, id, track_type)")
      .eq("student_id", studentId)
      .eq("status", "active")
  ...
  return buildStudentProfile({
    ...
    targetBand,
    placementBand,
    enrolledTrackSlug: enrolledLevel?.id ?? enrolledLevel?.slug ?? null,
