...
/**
 * Resolve word ids for a CEFR level, optionally filtered by topic.
 * Topic matching mirrors getTopicSummariesForLevel: case-insensitive, and
 * null/empty topic_category counts as "general".
 */
async function fetchLevelWordIds(supabase, level, topicCategory = null) {
  ...
  return rows
    .filter((row) => {
      const key =
        String(row.topic_category ?? "").trim().toLowerCase() || "general";
      return key === topic;
    })
    .map((w) => w.id);
}
 ...
 * Optional poolIds restricts the queue to a subset (e.g. one topic_category).
 */
async function buildStudyQueueInternal(
  supabase,
  studentId,
  cefrLevel,
  limit = VOCAB_SESSION_SIZE,
  poolIds = null
) {
  const level = normalizeSpeakifyCefrLevel(cefrLevel);
  const today = todayDateKey();
  const poolSet =
    poolIds && poolIds.length > 0 ? new Set(poolIds.map((id) => String(id))) : null;
  ...
  const addId = (id) => {
    ...
    if (poolSet && !poolSet.has(key)) return;
    seen.add(key);
    queueIds.push(key);
  };
...
export async function buildStudyQueue(supabase, studentId, cefrLevel, limit = VOCAB_SESSION_SIZE) {
  return buildStudyQueueInternal(supabase, studentId, cefrLevel, limit);
}

export async function buildTopicStudyQueue(
  supabase,
  studentId,
  cefrLevel,
  topicCategory,
  ...
  const level = normalizeSpeakifyCefrLevel(cefrLevel);
  const topic = String(topicCategory ?? "general").trim().toLowerCase();
  const poolIds = await fetchLevelWordIds(supabase, level, topic);
  const result = await buildStudyQueueInternal(
    supabase,
    studentId,
    level,
    limit,
  ...
  return {
    ...result,
    meta: { ...result.meta, topic },
  };
}

export async function getTopicSummariesForLevel(supabase, cefrLevel) {
  const level = normalizeSpeakifyCefrLevel(cefrLevel);
  let data = [];
  const modern = await supabase
    .from("vocabulary_words")
  ...
  if (!modern.error) {
    data = modern.data ?? [];
  } else if (isSchemaColumnError(modern.error)) {
    const fallback = await supabase
      .from("vocabulary_words")
      .select("id")
      .eq("cefr_level", level);
    if (fallback.error) throw fallback.error;
    data = (fallback.data ?? []).map(() => ({ topic_category: "general" }));
  } else {
    throw modern.error;
  }
  ...
  for (const row of data ?? []) {
    const key = String(row.topic_category ?? "general").trim().toLowerCase() || "general";
    counts.set(key, (counts.get(key) ?? 0) + 1);
  }

  return Array.from(counts.entries())
    .map(([key, wordCount]) => ({ key, wordCount }))
