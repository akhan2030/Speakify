...
export function normalizeListeningQuestionFields(q) {
  ...
  if (type === "multiple-choice") {
    return {
      ...q,
      options: normalizeListeningMcqOptions(q.options),
    };
  }
  ...
  if (type === "matching") {
    return {
      ...q,
      options: normalizeListeningMatchingOptions(q.options),
    };
  }
...
export function hydrateListeningQuestionsFromPayload(rawQuestions, answerKey) {
  ...
  return list.map((item, index) => {
    ...
    const num = Number(q.questionNumber ?? q.id ?? index + 1);
    const answerFromKey =
...
export function validateListeningQuestionContent(questions, sectionNumber = null) {
  ...
  for (const q of list) {
    const num = q.questionNumber ?? q.id ?? "?";
