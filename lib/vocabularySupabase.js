...
/** Columns on vocabulary_words (synonyms / arabic_equivalent optional until migrated). */
export const VOCAB_WORD_SELECT =
  "id, word, cefr_level, part_of_speech, definition, definition_arabic, arabic_equivalent, pronunciation_ipa, example_sentence, ielts_example, memory_hook, topic_category, synonyms";
...
export const VOCAB_WORD_SELECT_WITH_EQUIVALENT =
  "id, word, cefr_level, part_of_speech, definition, definition_arabic, arabic_equivalent, pronunciation_ipa, example_sentence, ielts_example, memory_hook, topic_category";
...
function looksLikeShortArabicEquivalent(text) {
...
export async function fetchVocabularyWordsByIds(supabase, wordIds) {
  ...
  const selects = [
    VOCAB_WORD_SELECT,
    VOCAB_WORD_SELECT_WITH_EQUIVALENT,
    VOCAB_WORD_SELECT_BASE,
  ];
  ...
  for (const columns of selects) {
    ...
    if (!msg.includes("synonyms") && !msg.includes("arabic_equivalent")) {
      break;
    }
  }
...
export function mapWordRow(row) {
  let arabic_equivalent = String(row.arabic_equivalent ?? "").trim();
  let definition_arabic = String(
    row.definition_arabic ?? row.arabic_translation ?? ""
  ).trim();

  // Legacy seed data stored short dictionary glosses in definition_arabic.
  if (!arabic_equivalent && looksLikeShortArabicEquivalent(definition_arabic)) {
    arabic_equivalent = definition_arabic;
    definition_arabic = "";
  }
