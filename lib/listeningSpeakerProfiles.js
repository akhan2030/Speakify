...
export function getVoiceForSpeakerLabel(speakerLabel, sectionNumber, speakers) {
  ...
  const customerRoles = new Set(["caller", "guest", "customer"]);

  if (staffRoles.has(key)) {
    return sectionNumber === 1 ? "onyx" : "fable";
  }
  if (customerRoles.has(key)) {
    return sectionNumber === 1 ? "nova" : "shimmer";
  }
  ...
  if (gender === "female") {
    ...
    return femaleVoices[idx];
  }

  const maleVoices = ["onyx", "echo", "fable"];
  const idx =
    Math.abs(key.split("").reduce((sum, ch) => sum + ch.charCodeAt(0), 0)) %
    maleVoices.length;
  return maleVoices[idx];
}
