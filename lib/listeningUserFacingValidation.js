...
export function assertSectionBlockRanges(sectionNumber) {
  ...
  const expectedStart = getSectionQuestionBlocks(1)[0]?.start ?? 1;
  ...
  for (const block of blocks) {
    ...
    if (block.end - block.start + 1 !== 5) {
      throw new Error(
        `Section ${sectionNumber} block must have 5 questions, got ${block.start}–${block.end}`
      );
    }
  }

  if (sectionNumber === 1 && blocks[0]?.start !== expectedStart) {
    throw new Error(
      `Section 1 first block must start at ${expectedStart}, got ${blocks[0]?.start}`
    );
  }
}

/**
 * UI/audio prep ranges must be 1–5 and 6–10 within each section (global numbers for S2+).
 * @param {number} sectionNumber
 */
export function validateInstructionRangesForSection(sectionNumber) {
  ...
  if (blocks.length < 2) {
    errors.push(`Section ${sectionNumber}: expected 2 question blocks`);
    return { valid: false, errors };
  }

  const [first, second] = blocks;
  const firstSpoken = formatSpokenQuestionRange(first.start, first.end);
  const secondSpoken = formatSpokenQuestionRange(second.start, second.end);
  ...
  if (first.end - first.start + 1 !== 5 || second.end - second.start + 1 !== 5) {
    errors.push(
      `Section ${sectionNumber}: blocks must be 5 questions each (${first.start}–${first.end}, ${second.start}–${second.end})`
    );
  }
...
export function validateListeningSectionPayload(
  ...
  const textsToScan = [
    formatSpokenQuestionRange(blocks[0]?.start ?? 1, blocks[0]?.end ?? 5),
    formatSpokenQuestionRange(blocks[1]?.start ?? 6, blocks[1]?.end ?? 10),
