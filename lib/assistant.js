...
import { syncWritingEvaluationScores } from "@/lib/ielts/writingBandScore.js";
import {
  TASK1_EVAL_RUBRIC,
  TASK2_EVAL_RUBRIC,
...
function applyWritingBandScores(evaluationText, taskType = "task2") {
  return syncWritingEvaluationScores(evaluationText, taskType);
}
...
export async function evaluateEssay(userEssay, taskType) {
  ...
  const lengthNote = underMinimum
    ? `IMPORTANT: Response is UNDER the IELTS minimum of ${minWords} words. Penalize ${firstAbbrev} accordingly and mention the under-length issue in feedback.\n\n`
    : "";
  await openai.beta.threads.messages.create(threadId, {
    ...
    content:
      `${rubric}\n\n---\n\nTask type: ${taskLabel}\n` +
      `${lengthNote}` +
      `Word count: ${wordCount} words (IELTS minimum for this task: ${minWords})\n\n` +
  ...
  const { evaluation: scoredEvaluation, bands: scoredBands } = applyWritingBandScores(
    rawEvaluation,
    taskType
  );
  let evaluation = scoredEvaluation;
  let bands = scoredBands;
  ...
  try {
    ...
    // Structured score is kept for roadmap analytics only.
    // Display bands stay aligned with the prose evaluation header.
  } catch (structuredErr) {
    console.warn(
