...
1. Score each criterion as a whole number or .5 only (e.g. 5, 5.5, 6, 6.5)
2. Overall band = average of 4 criteria rounded to nearest 0.5
3. If essay is under 250 words — penalise TA by at least 1 band
4. Be strict — an average essay with basic vocabulary is band 5 to 5.5
5. Band 7 requires genuinely good vocabulary and complex structures
6. Never give band 7+ unless the essay clearly demonstrates band 7 features
7. Be consistent — same essay must always receive same scores
...
function calculateOverall(ta, cc, lr, gra) {
...
function parseBandsFromEvaluation(evaluation) {
  const ta = parseCriterionScore(evaluation, "TA");
  const cc = parseCriterionScore(evaluation, "CC");
  const lr = parseCriterionScore(evaluation, "LR");
  const gra = parseCriterionScore(evaluation, "GRA");
  const overall = calculateOverall(ta, cc, lr, gra);
  return { ta, cc, lr, gra, overall };
}
...
export async function evaluateEssay(userEssay, taskType) {
  ...
  await openai.beta.threads.messages.create(threadId, {
    ...
    content:
      ...
      "Evaluate using the required OUTPUT FORMAT. Put TA/CC/LR/GRA scores on the first lines.\n\n" +
      trimmedEssay,
  });
  ...
  const evaluation = extractTextFromMessages(messages);

  if (!evaluation) {
    return "No evaluation text returned by assistant.";
  }

  const bands = parseBandsFromEvaluation(evaluation);
