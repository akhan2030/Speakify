...
import { hydrateListeningQuestionsFromPayload, coalesceQuestionOptionsRaw, normalizeListeningMcqOptions, coalesceQuestionText, coalesceQuestionNumber, coalesceQuestionAnswer, isListeningPlaceholderQuestion, normalizeListeningQuestionFields } from "./listeningQuestionContent.js";
import {
...
- Include distractors — wrong answers mentioned first then corrected: "Actually, no, it is..." / "sorry, actually..."
- Follow real IELTS section formats exactly
- Never use the same topic twice
- MCQ: exactly 3 options (A, B, C) — never 4
- Matching: answers are letters from a box, not free-text phrases
...
export const SECTION_CONFIG = {
  ...
  2: {
    ...
    type: "monologue",
    speakers: 1,
    primaryQuestionType: "plan-map-diagram",
    topics: [
      "Museum audio tour",
  ...
  4: {
    ...
    type: "lecture",
    speakers: 1,
    primaryQuestionType: "summary-completion",
    topics: [
...
/**
 * Repair MCQ options on normalized section payload (post-normalize pass).
 * Only touches questions whose planned type is multiple-choice.
 */
async function repairNormalizedMcqOptions(normalized, openai, sectionNumber) {
  const questions = Array.isArray(normalized?.questions)
    ? normalized.questions.map((q) => ({ ...q }))
    : [];

  const needsOptions = questions.filter((q) => {
    const qNum = coalesceQuestionNumber(q);
    if (qNum == null) return false;
    const planned = getTypeForQuestionNumber(sectionNumber, qNum);
    if (planned !== "multiple-choice") return false;
    return isListeningPlaceholderQuestion({ ...q, type: "multiple-choice" });
  });

  if (!needsOptions.length) return normalized;

  console.warn(
    `[listeningGenerator] repairing ${needsOptions.length} normalized MCQ question(s) in section ${sectionNumber}`
  );
    ...
    messages: [
      {
        role: "system",
        content:
          'Return JSON: {"questions":[{"questionNumber":21,"options":[{"label":"A","text":"..."},{"label":"B","text":"..."},{"label":"C","text":"..."}]}]}. Every option needs non-empty text grounded in the transcript.',
      },
      {
        role: "user",
        content: JSON.stringify({
          sectionNumber: sectionNumber,
          transcriptExcerpt: String(normalized.transcript ?? "").slice(0, 5500),
          questions: needsOptions.map((q) => ({
            questionNumber: coalesceQuestionNumber(q),
  ...
  const fixParsed = JSON.parse(completion.choices?.[0]?.message?.content ?? "{}");
  const fixes = Array.isArray(fixParsed.questions) ? fixParsed.questions : [];

  for (const fix of fixes) {
    const targetNum = Number(fix.questionNumber);
    const idx = questions.findIndex((q) => coalesceQuestionNumber(q) === targetNum);
    if (idx < 0 || !Array.isArray(fix.options) || fix.options.length < 3) continue;
    questions[idx] = normalizeListeningQuestionFields({
      ...questions[idx],
      type: "multiple-choice",
      options: fix.options.map((opt, optIndex) =>
        typeof opt === "string"
          ? opt
          : {
              label: opt.label ?? opt.key ?? ["A", "B", "C"][optIndex],
...
CRITICAL — spoken EXAMPLE (before Question ${globalStart}):
- Include an "example" object in the JSON with questionText + answer.
- The example answer MUST be spoken clearly in the opening of the transcript BEFORE any scored answers.
- After asking for the caller's name, the caller MUST reply with their name aloud (never go silent).
- Do NOT invent paper-only examples like "Harbour City" unless those exact words are spoken.`
      : "";

  const section3McqExample =
    sectionNumber === 3
      ? `
Example (Section 3 — Questions ${globalStart}–${globalStart + 4} MUST be multiple-choice with exactly 3 options each):
{
  "questionNumber": ${globalStart},
  "type": "multiple-choice",
  "text": "What do the students agree is the main challenge?",
  "options": [
    { "label": "A", "text": "Limited funding for equipment" },
    { "label": "B", "text": "Tight submission deadlines" },
    { "label": "C", "text": "Conflicting supervisor feedback" }
  ],
  "answer": "B",
  "explanation": "Student B explains the deadline pressure."
}
CRITICAL: Every multiple-choice question MUST include options A, B, and C with real answer text — NEVER "options": [] for MCQ.
Questions ${globalStart + 5}–${globalEnd} are matching — include a shared options list on the first matching question.`
      : "";
...
${buildAuthenticityPromptRules(sectionNumber)}
...
Return this exact JSON structure:
{
  ...
  "example": ${
    sectionNumber === 1
      ? '{ "questionText": "string (gap with .......................)", "answer": "string spoken in opening dialogue" }'
      : "null"
  },
  "questions": [
    {
      ...
      "text": "string",
      "options": ${resolvedPrimary === "multiple-choice" ? '[{ "label": "A", "text": "..." }, { "label": "B", "text": "..." }, { "label": "C", "text": "..." }]' : "[]"},
      "answer": "string",
      "wordLimit": "string",
      "explanation": "string"
    }
...
Every multiple-choice question MUST include all 3 options with real text — never an empty options array.

For matching questions (Section 3 questions 6–10 within the section, e.g. global 26–30):
- Each question MUST include a non-empty "answer" field (single letter A–E matching the option list).
- Provide a shared matching option list on the first matching question if needed.

Critical rules:
- EVERY question MUST include a non-empty "answer" field (use "answer", not "correct_answer" alone).
...
- Every answer must appear verbatim in the transcript (students hear every answer)
- After any question like "What's your name?", the other speaker MUST speak the name in the next turn
- Space answers: do not place consecutive gap-fill answers in the same short utterance — weave natural dialogue between them
- Distractors: wrong number/date/option first, then correction, for at least two tested items
- Paraphrase question ideas with synonyms — do not echo the written question stem
...
- Transcript length: Section 1 ~280-520 words, Section 2 ~320-580, Section 3 ~360-650, Section 4 ~400-720
- Include natural hesitations: er, um, well, actually — especially in Sections 1 and 3
- Insert [SECTION BREAK] between the two question blocks (Sections 1–3)
- Section 4: include [pause] / [long pause] between major points; NO speaker labels in the transcript text
- Invent a 100% original scenario — never copy commercial reference recordings`;
}
...
function normalizeSectionPayload(parsed, sectionNumber, options = {}) {
  ...
  const answerKey = parsed.answer_key ?? parsed.answerKey ?? null;
  const hydrated = hydrateListeningQuestionsFromPayload(questions, answerKey);

  const mappedQuestions = normalizeSectionQuestions(hydrated, sectionNumber);
...
export async function generateListeningSection({
  ...
  for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
    try {
      ...
      if (section === 3) {
        const hasPlannedMcq = getSectionQuestionBlocks(3).some(
          (b) => b.type === "multiple-choice"
        );
        if (hasPlannedMcq) {
          normalized = await repairNormalizedMcqOptions(
            normalized,
            openai,
            section
          );
      ...
      const authenticity = validateListeningAuthenticity(
        normalized.transcript,
        section,
        { questions: normalized.questions }
      );
      if (!authenticity.ok) {
        throw new Error(authenticity.errors.join("; "));
      }
...
export function scoreListeningAnswers(
  ...
  return scoreListeningAnswersWithFeedback(
    studentList.map((s) => String(s ?? "")),
    correctList.map((c) => String(c ?? "")),
    Array.isArray(questionMeta) ? questionMeta : []
  );
}
...
export function calculateListeningBand(rawScore, totalQuestions = 40) {
  return listeningRawToBand(rawScore, totalQuestions);
}
