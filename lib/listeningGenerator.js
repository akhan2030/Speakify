...
async function repairNormalizedMcqOptions(normalized, openai, sectionNumber) {
  const mcqStart = getSectionFirstQuestionNumber(sectionNumber);
  const mcqEnd = mcqStart + 4;
  ...
  const needsOptions = questions.filter((q) => {
    ...
    if (qNum == null || qNum < mcqStart || qNum > mcqEnd) return false;
...
export async function generateListeningSection({
  ...
  for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
    try {
      ...
      if (section === 3) {
        normalized = await repairNormalizedMcqOptions(normalized, openai, section);
      }
