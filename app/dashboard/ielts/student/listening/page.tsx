"use client";

import { Suspense, useEffect, useState } from "react";
import Link from "next/link";
import { useSession } from "next-auth/react";
import { PageSpinner } from "@/components/StudentSidebar";
import SkillBandHeader from "@/components/ielts/SkillBandHeader";
import SkillTabs from "@/components/ielts/SkillTabs";
import { ACADEMIC_LISTENING_FULL_TESTS } from "@/lib/listening/academicListeningTestList";

const BASE = "/dashboard/ielts/student/listening";

const TABS = [
  { id: "sections", label: "Section Practice" },
  { id: "full", label: "Full Listening" },
  { id: "accents", label: "Accent Exposure" },
  { id: "strategies", label: "Strategies" },
];

const SECTIONS = [
  {
    n: 1,
    title: "Conversation",
    desc: "Everyday social context — booking, enquiries.",
    color: "#c9972c",
  },
  {
    n: 2,
    title: "Social monologue",
    desc: "Tours, announcements, local facilities.",
    color: "#0d9488",
  },
  {
    n: 3,
    title: "Academic discussion",
    desc: "2–4 speakers — university assignments.",
    color: "#7c3aed",
  },
  {
    n: 4,
    title: "Academic lecture",
    desc: "Science, history, technology — hardest section.",
    color: "#1d4ed8",
  },
];

const ACCENTS = [
  {
    label: "British",
    desc: "Standard IELTS accent — most common in exams.",
    href: `${BASE}/section/1`,
  },
  {
    label: "Australian",
    desc: "Section 2 & 3 often use Australian speakers.",
    href: `${BASE}/section/2`,
  },
  {
    label: "American",
    desc: "Less common but appears in some recordings.",
    href: `${BASE}/section/3`,
  },
];

function ListeningContent() {
  const { data: session } = useSession();
  const studentId = (session?.user as { id?: string })?.id ?? "";
  const [overallBand, setOverallBand] = useState<number | null>(null);

  useEffect(() => {
    if (!studentId) return;
    fetch(`/api/listening/tracker?studentId=${encodeURIComponent(studentId)}`)
      .then((r) => r.json())
      .then((json) => {
        if (json?.overallBand != null) setOverallBand(json.overallBand);
      })
      .catch(() => {});
  }, [studentId]);

  return (
    <SkillTabs tabs={TABS} defaultTab="sections">
      {(tab) => {
        if (tab === "sections") {
          return (
            <div className="grid gap-4 sm:grid-cols-2">
              {SECTIONS.map((s) => (
                <div
                  key={s.n}
                  className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm"
                  style={{ borderLeftWidth: 4, borderLeftColor: s.color }}
                >
                  <p className="text-xs font-bold uppercase" style={{ color: s.color }}>
                    Section {s.n}
                  </p>
                  <h3 className="mt-1 font-bold text-[#0d1b35]">{s.title}</h3>
                  <p className="mt-2 text-sm text-slate-600">{s.desc}</p>
                  <p className="mt-2 text-xs text-slate-400">Audio plays once</p>
                  <Link
                    href={`${BASE}/section/${s.n}`}
                    className="mt-4 inline-block rounded-xl px-5 py-2.5 text-sm font-bold text-white hover:opacity-95"
                    style={{ backgroundColor: s.color }}
                  >
                    Practice Section {s.n} →
                  </Link>
                </div>
              ))}
            </div>
          );
        }
        if (tab === "full") {
          return (
            <div>
              <div className="mb-4">
                <h3 className="text-xl font-bold text-[#0d1b35]">Full 40-question papers</h3>
                <p className="mt-1 text-sm text-slate-600">
                  Five original 40-question papers, numbered in order. 30 minutes · audio once per section.
                </p>
                {overallBand != null ? (
                  <p className="mt-2 text-sm font-semibold text-[#c9972c]">
                    Your estimate: Band {overallBand.toFixed(1)}
                  </p>
                ) : null}
              </div>
              <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                {ACADEMIC_LISTENING_FULL_TESTS.map((test) => (
                  <div
                    key={test.n}
                    className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm"
                  >
                    <p className="text-xs font-bold uppercase tracking-wide text-[#c9972c]">
                      Test {test.n}
                    </p>
                    <h4 className="mt-1 font-bold text-[#0d1b35]">{test.title}</h4>
                    <p className="mt-2 text-sm text-slate-600">{test.summary}</p>
                    <Link
                      href={`${BASE}/test?test=${test.n}`}
                      className="mt-4 inline-block rounded-xl bg-[#0d1b35] px-5 py-2.5 text-sm font-bold text-white hover:opacity-95"
                    >
                      Start Test {test.n} →
                    </Link>
                  </div>
                ))}
              </div>
            </div>
          );
        }
        if (tab === "accents") {
          return (
            <div className="grid gap-4 md:grid-cols-3">
              {ACCENTS.map((a) => (
                <div
                  key={a.label}
                  className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm"
                >
                  <h3 className="font-bold text-[#0d1b35]">{a.label}</h3>
                  <p className="mt-2 text-sm text-slate-600">{a.desc}</p>
                  <Link
                    href={a.href}
                    className="mt-4 inline-block text-sm font-semibold text-[#0d9488] hover:underline"
                  >
                    Practice →
                  </Link>
                </div>
              ))}
            </div>
          );
        }
        return (
          <div className="space-y-4">
            {[
              {
                title: "Prediction before audio",
                text: "Read questions first — predict answers and note keywords.",
              },
              {
                title: "Note-taking shorthand",
                text: "Use symbols and abbreviations — don't write full sentences while listening.",
              },
              {
                title: "Spelling counts",
                text: "Wrong spelling = wrong answer. Practice common IELTS vocabulary spelling.",
              },
              {
                title: "Section 4 strategy",
                text: "Lecture format — follow signpost words: however, in conclusion, the main point.",
              },
            ].map((tip) => (
              <div
                key={tip.title}
                className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm"
              >
                <h3 className="font-bold text-[#0d1b35]">{tip.title}</h3>
                <p className="mt-1 text-sm text-slate-600">{tip.text}</p>
              </div>
            ))}
            <Link
              href={`${BASE}/tracker`}
              className="inline-block text-sm font-semibold text-[#0d9488] hover:underline"
            >
              View full listening tracker →
            </Link>
          </div>
        );
      }}
    </SkillTabs>
  );
}

export default function IeltsListeningPage() {
  return (
    <main className="min-h-screen flex-1 bg-slate-50 p-4 pb-24 md:p-6 md:pb-6">
      <div className="mb-4 flex items-start gap-3 rounded-xl bg-[#c9972c]/15 px-4 py-3 text-sm text-[#0d1b35]">
        <span>🎧</span>
        <span>
          Use headphones. Audio plays <strong>once</strong> — real exam conditions.
          Answer checking includes explanations after each session.
        </span>
      </div>
      <SkillBandHeader
        skill="listening"
        title="Listening"
        subtitle="Section practice, full tests, and accent exposure"
      />
      <Suspense fallback={<PageSpinner />}>
        <ListeningContent />
      </Suspense>
    </main>
  );
}
