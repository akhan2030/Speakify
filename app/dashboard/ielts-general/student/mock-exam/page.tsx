...
import { gtMockExamHref } from "@/lib/mock-test/ieltsMockRoutes";
...
const PRE_MOCK_TIPS = [
  ...
  "Remember: Task 1 is a letter, not a graph",
];
...
function catalogFallbackMocks(highlightMock?: number): MockItem[] {
  return GT_MOCK_CATALOG.map((item) => ({
    id: null,
...
function buyHref(mockNumber: number) {
  return `/checkout/mock?product=single&mock=${mockNumber}&programme=ielts_general`;
}

function pack3Href() {
  return `/checkout/mock?product=pack3&programme=ielts_general`;
}
...
export default function GtMockExamLobbyPage() {
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        const res = await fetch("/api/ielts-general/mock-exam/page-data");
        ...
        if (dataRes.error && !Array.isArray(dataRes.availableMocks)) {
          ...
          setPageData({
            ...
            readinessPercent: 0,
            bandTrend: "none",
            currentMockNumber: Number.isInteger(requestedMockNumber)
              ? requestedMockNumber
              : 1,
            lastMock: null,
            history: [],
  ...
  async function handleStartMock(mock: MockItem) {
    ...
    try {
      ...
      router.push(
        gtMockExamHref({ mockNumber: mock.mockNumber, attemptId })
      );
    } catch (err) {
      console.error("[gt-mock-exam] start failed:", err);
  ...
  useEffect(() => {
    ...
    if (
      !Number.isInteger(requestedMockNumber) ||
      requestedMockNumber < 1 ||
      requestedMockNumber > 3
    ) {
      return;
    }
    const mock = pageData.availableMocks.find(
      (m) => m.mockNumber === requestedMockNumber
    );
  ...
  const currentMock =
    mocks.find((m) => m.isCurrent) ?? mocks.find((m) => m.status === "available");
  const ownedCount = pageData?.access?.purchasedMockNumbers?.length ?? 0;
  const hasAllMocks = pageData?.access?.hasAllMocks === true;
  const sellableCount = pageData?.access?.sellableCount ?? GT_MOCK_CATALOG.length;
  const showPack3Upgrade = !hasAllMocks && ownedCount > 0 && ownedCount < sellableCount;
  const startableCount = mocks.filter(
    (m) => m.canStart === true && m.status !== "completed"
  ).length;
      ...
      <div className="lg:flex lg:items-start lg:gap-8">
        <div className="min-w-0 flex-1">
          ...
          <p
            ...
            style={{ color: TEAL }}
          >
            General Training exam simulation
          </p>
          ...
          <div className="space-y-4">
            ...
            {mocks.map((mock) => (
              <MockExamCard
                ...
                showPack3Upgrade={showPack3Upgrade}
              />
            ))}
          </div>
