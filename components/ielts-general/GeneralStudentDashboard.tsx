...
export default function GeneralStudentDashboard() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const res = await fetch("/api/ielts-general/dashboard");
  ...
  async function saveExamDate() {
    ...
    setSavingExam(true);
    try {
      await fetch("/api/ielts-general/dashboard", {
        method: "PATCH",
  ...
  return (
    <main className="min-h-screen flex-1 bg-slate-50 p-4 pb-24 md:p-6 md:pb-6">
      <div className="mb-4">
        <h1 className="text-2xl font-bold text-[#0d1b35] md:text-3xl">
          {data.programLabel ?? "IELTS General Training"}
        </h1>
        <p className="mt-1 text-sm text-slate-500">
          Letter, essay, and everyday English skills
        </p>
      </div>
      ...
      <div className="mb-6 flex flex-wrap items-center justify-between gap-3 rounded-xl border border-slate-200 bg-white px-4 py-3 text-sm shadow-sm">
        ...
        <div>
          {data.exam.daysRemaining != null ? (
            ...
            <Link
              href="/dashboard/ielts-general/student/settings"
      ...
      <div className="mb-6 grid gap-6 lg:grid-cols-5">
            ...
            <div className="mt-6 space-y-4">
              {data.bands.skills.map((skill) => (
                <div key={skill.key} className="flex flex-wrap items-center gap-3 text-sm">
                  <span className="min-w-[9rem] font-medium text-[#0d1b35]">{skill.label}</span>
        ...
        <div className="space-y-6 lg:col-span-2">
          ...
          <div className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
            <h3 className="text-sm font-bold text-[#0d1b35]">Letter type accuracy</h3>
            <p className="mt-1 text-xs text-slate-500">Recent formal, semi-formal & informal letters</p>
            <dl className="mt-4 grid grid-cols-3 gap-2 text-center">
              <div className="rounded-lg bg-slate-50 px-2 py-3">
                <dt className="text-[10px] font-semibold uppercase tracking-wide text-slate-500">
                  Formal
                </dt>
                <dd className="mt-1 text-lg font-bold text-[#0d1b35]">
                  {formatAccuracy(data.letterTypeAccuracy?.formal)}
                </dd>
              </div>
              <div className="rounded-lg bg-slate-50 px-2 py-3">
                <dt className="text-[10px] font-semibold uppercase tracking-wide text-slate-500">
                ...
                <dd className="mt-1 text-lg font-bold text-[#0d1b35]">
                  {formatAccuracy(data.letterTypeAccuracy?.semiFormal)}
                </dd>
              </div>
              <div className="rounded-lg bg-slate-50 px-2 py-3">
                <dt className="text-[10px] font-semibold uppercase tracking-wide text-slate-500">
                ...
                <dd className="mt-1 text-lg font-bold text-[#0d1b35]">
                  {formatAccuracy(data.letterTypeAccuracy?.informal)}
                </dd>
              </div>
            </dl>
            <Link
              href={`${BASE}/letter-practice`}
              className="mt-3 block text-center text-xs font-semibold text-[#0d9488] hover:underline"
            >
              Practice letters →
            </Link>
          </div>
