/**
 * Adds payment columns + payment_transactions on production Postgres.
 * Requires SUPABASE_DB_URL, DATABASE_URL, or SUPABASE_DB_PASSWORD in .env.local
 *
 * Run: node scripts/applyPaymentMigration.js
 */
const fs = require("fs");
...
async function main() {
  ...
  if (!connectionString) {
    ...
    console.error(
      "Alternatively, paste supabase/payment_setup.sql into the Supabase SQL Editor."
    );
    process.exit(1);
  }

  const sql = fs.readFileSync(
    path.join(__dirname, "..", "supabase", "payment_setup.sql"),
    "utf8"
  );
  ...
  try {
    ...
    const { rows } = await client.query(
      `SELECT column_name FROM information_schema.columns
       WHERE table_schema = 'public' AND table_name = 'users' AND column_name = 'payment_status'`
    );
    if (rows.length) {
      console.log("OK  users.payment_status column exists on production");
    } else {
